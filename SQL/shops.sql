...
Group by
  sname;
-- найти продавца из Лондона у которого самые маленьокие комиссионные
SELECT
  sname
FROM salepeople
WHERE
...
GROUP BY
  CITY;
--объедщенить все таблицы в одну
Select
  *
from customers,
  orders,
  salepeople
where
  salepeople.snum = orders.snum
  AND orders.cnum = customers.cnum --рассчитать средний реётинг покупателей для каждого продавца
Select
  SNAME,
  avg(RATING)
from customers,
...
Group by
  sname,
  ODATE;
-- найти среднюю сумму заказов на каждое число
SELECT
  ODATE,
  avg(amt)
FROM orders
